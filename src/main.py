...
def _best_combo_from_single_fixture(
    ...
    for L in range(max(3, min_legs), max_legs + 1):
        ...
        for legs, prod, st in beam:
            ...
            diff = math.log(max(0.0001, prod)) - log_target
            score = abs(diff) + (0.03 * len(legs)) + (1e-6 if diff < 0 else 0.0)  # empate: prefere passar de 3
...
def greedy_select_legs_for_target(
    ...
    while len(legs) < max_legs:
        # score atual (sem adicionar nova perna)
        cur_diff = math.log(max(0.0001, prod)) - log_target
        cur_score = abs(cur_diff) + (0.04 * len(legs)) + (1e-6 if cur_diff < 0 else 0.0)
        ...
        # só adiciona se melhorar (ou empatar dentro de tolerância) a proximidade do target
        if best_score is None or best_score >= (cur_score - 1e-12):
            break
 
        # agora aplica de verdade
        if not can_add(best):  # deveria ser raro, mas por segurança
            cands.remove(best)
            continue
        legs.append(best)
